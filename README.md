...
- **新增测试**：`_test_usage_range.py`（22 项断言，时间范围过滤语义、缓存隔离、与 analytics 口径一致性）、`_test_matrix_filters.js`（19 项断言，以 Node 驱动真实 dashboard 代码验证筛选、汇总重算与失效应答）、`_test_api_key_models.py`（39 项断言：模型范围的规范化与向后兼容、`/v1/models` 过滤、两个对话端点的拦截与放行、面板保存与回读）与 `_test_key_model_picker.js`（17 项断言，以 Node 驱动真实 dashboard 代码验证勾选渲染、读写与失效应答）。全部测试合计 346 项断言通过。
